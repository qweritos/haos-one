...
- [x] 5.4 Publish self-contained host executables named `haos-one-net-mac-intel`, `haos-one-net-mac-apple-silicon`, and `haos-one-net-windows.exe`, with a separate `.sha256` checksum for each, alongside the container images; run userspace WireGuard in-process with no companion executable or archive extraction.
